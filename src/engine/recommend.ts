...
export interface EmblemPickResult { emblemId: string; grade: EmblemGrade; }
...
export function unneededStats(pokemon: Pokemon): Set<keyof StatBlock> {
  ...
  if (pokemon.attackType === "physical") { set.add("spAttack"); set.add("spLifesteal"); }
  if (pokemon.attackType === "special") { set.add("attack"); }
...
const NEG_FLOOR: Partial<Record<keyof StatBlock, number>> = {
  hp: -210, attack: -15, defense: -16, spAttack: -15, spDefense: -16, moveSpeed: -210,
};

const STAT_VALUE_SCALE: Record<keyof StatBlock, number> = {
  hp: 200, attack: 14, defense: 14, spAttack: 14, spDefense: 14,
  critRate: 0.04, cdr: 0.08, lifesteal: 0.05, spLifesteal: 0.05, attackSpeed: 0.09, moveSpeed: 150,
};
...
export function colorCountsOf(picks: { emblemId: string }[], byId: Map<string, Emblem>): Map<EmblemColor, number> {
...
export function colorTargetsFor(pokemon: Pokemon, byId: Map<string, Emblem>): Map<EmblemColor, number> {
  ...
  if (pokemon.role === "Defender") return new Map([["white", 6], ["brown", physical ? 4 : 0], ["blue", 2]].filter(([, n]) => (n as number) > 0) as [EmblemColor, number][]);
  if (pokemon.role === "Supporter") return new Map<EmblemColor, number>([["black", 6], ["white", 4]]);
  if (physical) return new Map<EmblemColor, number>([["brown", 6], ["white", 6]]);
  return new Map<EmblemColor, number>([["green", 6], ["black", 6]]);
}
...
export function solveEmblemSet(
  pokemon: Pokemon,
  emblems: Emblem[],
  opts: { weights?: Partial<Record<keyof StatBlock, number>>; targets?: Map<EmblemColor, number>; owned?: Set<string>; seed?: number } = {},
): EmblemPickResult[] {
...
const SCALE: Record<keyof StatBlock, number> = {
  hp: 240, attack: 18, defense: 16, spAttack: 30, spDefense: 16,
  critRate: 0.04, cdr: 0.09, lifesteal: 0.06, spLifesteal: 0.06, attackSpeed: 0.105, moveSpeed: 175,
};
...
export function priorityWeights(pokemon: Pokemon): Partial<Record<keyof StatBlock, number>> {
  ...
  if (physical) { add("attack", 3); add("critRate", 1.5); add("attackSpeed", 1.2); add("lifesteal", 0.8); }
  if (special) { add("spAttack", 3); add("cdr", 1.5); add("spLifesteal", 0.6); }

  switch (pokemon.role) {
    case "Attacker": add(physical ? "attack" : "spAttack", 1.5); add("critRate", physical ? 0.8 : 0); break;
    case "AllRounder": add("attack", 0.8); add("hp", 0.8); add("lifesteal", 0.6); break;
    case "Speedster": add("attack", 1); add("critRate", 1); add("moveSpeed", 0.6); add("lifesteal", 0.6); break;
    case "Defender": add("hp", 2.5); add("defense", 2); add("spDefense", 2); add("cdr", 1); break;
    case "Supporter": add("cdr", 2); add("hp", 1.5); add("spDefense", 1.2); break;
  }
  return w;
}
...
export function scoreHeldItem(
  ...
  if (offensive && item.conditionalEffects.some((e) => e.stacking || e.type === "onBasicAttack")) score += 0.5;
...
const STAT_COLOR: Partial<Record<keyof StatBlock, EmblemColor>> = {
  attack: "brown", spAttack: "green", defense: "blue", spDefense: "purple",
  hp: "white", attackSpeed: "red", cdr: "black", moveSpeed: "yellow",
};
...
export function recommendBuild(
  ...
  for (const [stat] of ranked.slice(0, 2)) {
    ...
    const thresholds = Object.keys(def.thresholds).map(Number).sort((a, b) => b - a);
  ...
  const battleItemId = pokemon.role === "Defender" || pokemon.role === "Supporter" ? "eject-button" : "x-attack";
