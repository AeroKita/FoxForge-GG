...
App version shown in Settings → Updates comes from the `"version"` field in `package.json` (currently `1.4.2`), injected at build time via `vite.config.ts` (`define.__APP_VERSION__`) into `src/ui/version.ts` (`APP_VERSION`). Bump with `npm version <semver> --no-git-tag-version` (or edit `package.json` and sync `package-lock.json`).
...
| Pickers / settings | `PickerModal` (`BottomSheet fillHeight`; search does not auto-focus on open; optional `onClear` empty-slot tile; held picker `tip` via `itemTip(i, heldItemGrade(i.id))`; trainer `itemTip`; emblem grade-aware `tipForGrade`/`subtitleForGrade`), `SettingsMenu` (content-fit `BottomSheet`; read-only patch version + app version) |
