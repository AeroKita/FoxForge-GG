...
Game facts live in zod-validated JSON loaded via `src/data/loadBundle.ts`, with lookup maps exposed via `src/data/gameData.ts`. The build-time baseline is `src/data/patch-current.json` (stable filename; `patchVersion` is inside the JSON; 96 playable Pokémon as of `lastUpdated` 2026-07-04). The published runtime copy under `public/data/` is version-stamped (`patch-<version>.json` plus `manifest.json`) for cache-busting and is mirrored from the baseline by `tools/community/publish_bundle.py` (`npm run data:publish`; CI via `data.yml`). Numeric data is refreshed by Python tooling under `tools/community/`—hand-editing bundle JSON is discouraged; curated builds and label overrides belong in `curated_builds.json` (see below). `normalize.py` builds two description tiers per move and passive: Basic in `description` (UNITE-DB text from `_raw/`, blank-filled from the owned, hand-maintained `move_descriptions.json` when empty—with trailing `Activates at Level N` stripped by `strip_activation_note()`—plus upgrade bonus lines on Move 1/2 upgrades unless `description2` is cleared for verbatim upgrade text) and optional Advanced in `descriptionAdvanced` (UNITE-DB `rsb` detailed text via `advanced_desc()`). Unite moves also surface unlock level as `upgradeLevel` from the raw skill's `level` (`MANUAL_LEVEL` covers rare UNITE-DB gaps). Blank passive Basic text uses `passive_basic_desc()` — same fallback chain as moves: UNITE-DB `description`, then `move_descriptions.json` (via `strip_activation_note`), then `rsb.true_desc`. Source mirrors: `tools/community/_raw/pokemon.json` and `_raw/stats.json`. Art mirrors under `public/assets/` and resolves portably via `src/ui/asset.ts` (supports relative `base: "./"` for static hosts and GitHub Pages sub-paths).
...
App version shown in Settings → Updates comes from the `"version"` field in `package.json` (currently `2.1.0`), injected at build time via `vite.config.ts` (`define.__APP_VERSION__`) into `src/ui/version.ts` (`APP_VERSION`). Bump with `npm version <semver> --no-git-tag-version` (or edit `package.json` and sync `package-lock.json`).
...
| `python3 -m unittest tools/community/test_diff_bundle.py tools/community/test_normalize.py tools/community/test_harvest.py tools/community/test_publish_bundle.py tools/community/test_curate.py` | Semantic bundle-diff changelog (`diff_bundle.py`, including Basic description blanking warnings), `normalize.py` helpers (including `passive_basic_desc`, `strip_activation_note`, `build_upgrade_move`), `harvest_descriptions.py` archive merge, `publish_bundle.py` mirror/check, and `curate.py` scaffold/validation |
...
| Data pipeline (maintainer) | `tools/community/refresh.py`, `publish_bundle.py`, `harvest_descriptions.py`, `diff_bundle.py`, `doctor.py`, `curate.py`, `gaps.py` — orchestration and guardrails; `npm run data:*` in `package.json` |
