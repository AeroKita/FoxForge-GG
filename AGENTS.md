...
App version shown in Settings → Updates comes from the `"version"` field in `package.json` (currently `2.0.2`), injected at build time via `vite.config.ts` (`define.__APP_VERSION__`) into `src/ui/version.ts` (`APP_VERSION`). Bump with `npm version <semver> --no-git-tag-version` (or edit `package.json` and sync `package-lock.json`).
...
| Build UI helpers | `src/components/HeroBand.tsx`, `LoadoutBoard.tsx` (tappable `{name} · {grade}` slot captions), `StatDock.tsx`, `MyBuildsSheet.tsx`, `EffectiveStatsGrid.tsx`; pure UI helpers `src/ui/offense.ts`, `src/ui/setProgress.ts`, `src/ui/radarData.ts` |
