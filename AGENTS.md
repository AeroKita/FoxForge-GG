...
Human-oriented deep dives live in `docs/` (architecture, calculation engine, data sourcing, distribution, branding). [`docs/11-adding-content.md`](docs/11-adding-content.md) is the task-oriented maintenance runbook for adding Pokémon, items, builds, or clips; this file remains the architecture authority. [`CONTRIBUTING.md`](CONTRIBUTING.md) is the step-by-step guide for human contributors (setup, TDD workflow, `npm run verify`, PR checklist). GitHub auto-fills [`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md) on new PRs (conventional-commit title, UI screenshots, test notes). When behavior is ambiguous, those docs and engine validation tests are the source of truth—not comments in components.
