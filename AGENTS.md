...
- Tune each held item's grade (1–40) — tap-to-type via shared `GradeField` on the Held Items page (list rows with −/+ steppers) and in the Builder via `LoadoutBoard` grade chips → grade `BottomSheet`; pick each Pokémon's two final (upgraded) moves; both feed the live stats. Unique Items (Mega Stones and Rusted Sword) have no grade stats or grade UI on either surface (`isUniqueHeldItem` in `gameData.ts`).
...
Game facts live in zod-validated JSON loaded via `src/data/loadBundle.ts`, with lookup maps exposed via `src/data/gameData.ts`. The build-time baseline is `src/data/patch-current.json` (stable filename; `patchVersion` is inside the JSON; 95 playable Pokémon as of `lastUpdated` 2026-06-24). The published runtime copy under `public/data/` is version-stamped (`patch-<version>.json` plus `manifest.json`) for cache-busting and is mirrored from the baseline by `tools/community/publish_bundle.py` (`npm run data:publish`; CI via `data.yml`). Numeric data is refreshed by Python tooling under `tools/community/`—hand-editing bundle JSON is discouraged; curated builds and label overrides belong in `curated_builds.json` (see below). `normalize.py` builds two description tiers per move and passive: Basic in `description` (UNITE-DB text from `_raw/`, blank-filled from the owned, hand-maintained `move_descriptions.json` when empty—with trailing `Activates at Level N` stripped by `strip_activation_note()`—plus upgrade bonus lines on Move 1/2 upgrades unless `description2` is cleared for verbatim upgrade text) and optional Advanced in `descriptionAdvanced` (UNITE-DB `rsb` detailed text via `advanced_desc()`). Unite moves also surface unlock level as `upgradeLevel` from the raw skill's `level` (`MANUAL_LEVEL` covers rare UNITE-DB gaps). Blank passive Basic text is backfilled from `rsb.true_desc` when UNITE-DB's top-level `description` is empty. Source mirrors: `tools/community/_raw/pokemon.json` and `_raw/stats.json`. Art mirrors under `public/assets/` and resolves portably via `src/ui/asset.ts` (supports relative `base: "./"` for static hosts and GitHub Pages sub-paths).
...
Curated Recommended/Creative builds and build-label overrides live in `tools/community/curated_builds.json` and are merged by `normalize.py` (`apply_curated_builds`) after UNITE-DB normalization. `tools/community/curate.py` (`npm run data:curate`) scaffolds new per-Pokémon entries and validates the full file by importing `_validate_curated_build` from `normalize.py` before normalize runs. `tools/community/gaps.py` (`npm run data:gaps`) reports per-Pokémon missing Recommended builds, blank Basic descriptions, and missing clip/GIF assets on the live bundle. **Do not hand-edit `emblemName` in patch JSON** — regeneration will clobber it. Instead:
...
- **Build screen** — `BuildScreen` composes `HeroBand` (role-tinted band: portrait, name/role/attack-type chips, level slider; empty state = "Choose your Pokémon" CTA; opens `PokemonPickerSheet` with local state), `RecommendPanel` (returns `null` when no Pokémon — HeroBand is the sole empty-state voice), `LoadoutBoard` (single always-open card: held/trainer slot tiles → `PickerModal`, grade chips → grade `BottomSheet`, 10-position emblem cluster → per-emblem `BottomSheet` with grade selector + remove, set-progress chips from `src/ui/setProgress.ts` — unmet chips open the emblem picker with that color pre-filtered via `PickerModal` `initialFilterLabel`, net-flats line with `STAT_LABEL` short labels, bookmark → `MyBuildsSheet`), `MovesCard`, `StatPanel` (Advanced-only cards; Effective Stats renders shared `EffectiveStatsGrid`), lazy `LevelGraph` (card title **Stats Charts**; recharts not in the initial bundle), and `StatDock` (role-tinted via `ROLE_BAND` composited over opaque `bg-surface`, fixed strip above the tab bar: HP / dynamic offense / Move Speed / Crit Rate with 1.5s delta flashes suppressed for 800 ms after a Pokémon switch; tap opens a full-stats `BottomSheet` with `EffectiveStatsGrid`). The app-bar icon or title tap also opens `PokemonPickerSheet` (separate instance from Hero Band).
...
Human-oriented deep dives live in `docs/` (architecture, calculation engine, data sourcing, distribution, branding). [`docs/11-adding-content.md`](docs/11-adding-content.md) is the task-oriented maintenance runbook for adding Pokémon, items, builds, or clips — its **Quick start** section covers the two most common maintainer paths (balance refresh and new Pokémon); this file remains the architecture authority. [`CONTRIBUTING.md`](CONTRIBUTING.md) is the step-by-step guide for human contributors (setup, TDD workflow, `npm run verify`, PR checklist). GitHub auto-fills [`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md) on new PRs (conventional-commit title, UI screenshots, test notes). When behavior is ambiguous, those docs and engine validation tests are the source of truth—not comments in components.
...
Key scripts (from `package.json`): `npm run dev`, `npm run build`, `npm run build:pages`, `npm run typecheck`, `npm run lint`, `npm run lint:fix`, `npm run format`, `npm run format:check`, `npm run verify`. Data maintenance: `npm run data:doctor`, `data:refresh` (modes `full` / `curate` / `clips`), `data:curate`, `data:gaps`, `data:publish`, `data:publish:check` — see [`docs/11-adding-content.md`](docs/11-adding-content.md).

App version shown in Settings → Updates comes from the `"version"` field in `package.json` (currently `2.0.0`), injected at build time via `vite.config.ts` (`define.__APP_VERSION__`) into `src/ui/version.ts` (`APP_VERSION`). Bump with `npm version <semver> --no-git-tag-version` (or edit `package.json` and sync `package-lock.json`).
...
| `python3 -m unittest tools/community/test_diff_bundle.py tools/community/test_normalize.py tools/community/test_publish_bundle.py tools/community/test_curate.py` | Semantic bundle-diff changelog (`diff_bundle.py`), `normalize.py` helpers, `publish_bundle.py` mirror/check, and `curate.py` scaffold/validation |
...
`refresh.py` orchestrates the existing `tools/community/` scripts in order (`full` runs doctor → fetch → transcode → normalize → presets → art → boosts → publish → verify; `curate` and `clips` run smaller subsets). `normalize.py` writes `src/data/patch-current.json`, backfilling blank Basic move text from the owned `move_descriptions.json`; `publish_bundle.py` mirrors it to `public/data/` and updates `manifest.json`. Edit `curated_builds.json` (`_emblemNameRemap`, `_emblemNamePrefixRemap`, per-Pokémon `builds`/`creativeBuilds`/`recommendedTitles`) before re-running — never hand-edit curated labels in the bundle; validate with `npm run data:curate -- check`. Use a per-Pokémon `builds` overlay (not `recommendedTitles`) when both display order and labels must stay pinned against UNITE-DB reordering. Bump the patch id via `npm run data:refresh -- --patch-version X` or the workflow's optional `patch_version` input.
...
| Build UI helpers | `src/components/HeroBand.tsx`, `LoadoutBoard.tsx`, `StatDock.tsx`, `MyBuildsSheet.tsx`, `EffectiveStatsGrid.tsx`; pure UI helpers `src/ui/offense.ts`, `src/ui/setProgress.ts`, `src/ui/radarData.ts` |
...
| Compare tab (Advanced) | `src/components/screens/CompareScreen.tsx` → `CompareView` (lazy-loaded in `App.tsx`; normalized radar chart via `radarRows`; preset title via `MarqueeText` in `SidePicker`; Compare source label **Preset**); preset/loadout resolution in `src/state/compareBuilds.ts` (unit-tested) |
...
| Grade input | `src/components/GradeField.tsx` — shared tap-to-type grade field (`HeldItemsInventory`, `LoadoutBoard` grade sheet) |
...
| Data pipeline (maintainer) | `tools/community/refresh.py`, `publish_bundle.py`, `doctor.py`, `curate.py`, `gaps.py` — orchestration and guardrails; `npm run data:*` in `package.json` |
